...
#  (auto-loads RDS cache or builds from data/MC1_graph.json)
# ──────────────────────────────────────────────────────────────────────────
...
# -------------- vectors used by UI / JS -----------------------------------
tracks <- c("song1.mp3", "song2.mp3", "song3.mp3")   # must live in ./www/
...
all_edge_type_colors <- c(
  ...
  LyricistOf         = "#59a14f",
  RecordedBy         = "#edc949",
  InterpolatesFrom   = "#af7aa1",
  InStyleOf          = "#76b7b2",
  LyricalReferenceTo = "#ff9da7",
  CoverOf            = "#9c755f",
  DirectlySamples    = "#bab0ac",
  DistributedBy      = "#d37295",
  MemberOf           = "#8cd17d",
  default            = "#999999"
)
...
if (file.exists(cache_rds)) {
  message("Loading cached tables from ", cache_rds)
  list2env(readRDS(cache_rds), .GlobalEnv)
  
} else {
  if (!file.exists(json_path))
    stop("Cannot find ", json_path, " — please place the JSON in data/")
  
  message("Building tables from JSON …")
  ...
  nodes_tbl <- as_tibble(kg$nodes) %>%
    ...
    mutate(
      ...
      notable     = coalesce(notoriety, FALSE)  # if column exists
    )
  
  # ensure columns exist
  for (col in c("stage_name","genre","notable")) {
    if (!col %in% names(nodes_tbl)) nodes_tbl[[col]] <- NA
  }
  ...
  # ---------- helper --------------------------------------------------------
  sailor_id <- nodes_tbl %>%
    filter(tolower(name) == "sailor shift") %>%
    pull(id) %>% first()
  ...
  inward_ids <- unique(c(
    ...
    edges_tbl %>% filter(to %in%
                           (edges_tbl %>% filter(to == sailor_id) %>% pull(from))) %>% pull(from)
  ))
  
  outward_ids <- unique(c(
    sailor_id,
    edges_tbl %>% filter(from == sailor_id) %>% pull(to),
    edges_tbl %>% filter(from %in%
                           (edges_tbl %>% filter(from == sailor_id) %>% pull(to))) %>% pull(to)
  ))
  
  make_vis_nodes <- function(ids) {
    nodes_tbl %>%
      filter(id %in% ids) %>%
      mutate(
        label             = name,
  ...
  nodes_inward_vis  <- make_vis_nodes(inward_ids)
  edges_inward_final<- make_vis_edges(inward_ids)
  nodes_outward_vis <- make_vis_nodes(outward_ids)
  edges_outward_final<-make_vis_edges(outward_ids)
  ...
  saveRDS(list(
    ...
    edges_inward_final   = edges_inward_final,
    nodes_outward_vis    = nodes_outward_vis,
    edges_outward_final  = edges_outward_final,
    nodes_all_vis        = nodes_all_vis,
    edges_all_final      = edges_all_final),
    cache_rds)
  message("Tables cached to ", cache_rds)
}
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    # ---- JavaScript (audio + nav highlight) ------------------------------
    tags$script(HTML(sprintf("
      document.addEventListener('DOMContentLoaded',function(){
        ...
        playBtn.onclick=function(){
          ...
          ico.classList.toggle('fa-play-circle');
          ico.classList.toggle('fa-pause-circle');
        };
        nextBtn.onclick=function(){
          idx=(idx+1)%%tracks.length; p.src=tracks[idx]; p.play().catch(()=>{});
          if(ico.classList.contains('fa-play-circle')){
            ico.classList.toggle('fa-play-circle');
            ico.classList.toggle('fa-pause-circle');
          }
        };
...
# ════════════════════════════════════════════════════════════════════════════
# 3.  SERVER  (identical to your last working version)
# ════════════════════════════════════════════════════════════════════════════
server <- function(input, output, session){
  ...
  output$sailor_network <- renderVisNetwork({
    ...
    visNetwork(dat$nodes, dat$edges) %>%
      visNodes(
        color=list(background=~color.background,border="black",
